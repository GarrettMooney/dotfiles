...
```bash
source ~/.upstash && bun run scripts/upstash-client.ts <command> [args...]
```

**IMPORTANT**:
- Always run with `bun run`, not directly.
- **Always prefix every invocation with `source ~/.upstash &&`** so the script picks up the current credentials. The session's env vars may be stale (the Upstash database can be re-provisioned with a new URL/token); `~/.upstash` is the single source of truth.

## Configuration

### Credentials source

Credentials live in `~/.upstash`, a shell-sourceable file:
...
The script reads `UPSTASH_REDIS_REST_URL` and `UPSTASH_REDIS_REST_TOKEN` from env. Sourcing `~/.upstash` immediately before each invocation guarantees fresh values.

**Do NOT hardcode the URL or token in skill files, scripts, or anywhere else.** Anything that needs Upstash credentials sources `~/.upstash` at invocation time.

### Overriding credentials

If the user provides credentials from another source (conversation context, a different file), use the `--url` and `--token` flags to override env vars:

```bash
bun run scripts/upstash-client.ts --url "https://..." --token "..." GET mykey
```

**Priority**: Command-line flags > Environment variables (sourced from `~/.upstash`)
